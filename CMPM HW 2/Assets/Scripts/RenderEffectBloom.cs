...
public class RenderEffectBloom : MonoBehaviour
{
    ...
    void OnRenderImage(RenderTexture sourceTexture, RenderTexture destTexture)
    {
        if (BloomShader != null)
        {
            ...
            var brightPass = RenderTexture.GetTemporary(Screen.width, Screen.height);
            var blurPass = RenderTexture.GetTemporary(Screen.width, Screen.height);
            ScreenMat.SetFloat("_BloomThreshold", BloomThreshold);
    ...
    public void SetBloomFactor(float value)
    {
        BloomFactor = value;
    }
